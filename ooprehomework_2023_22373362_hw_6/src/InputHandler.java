...
public class InputHandler {
    ...
    public void OP2(int i) {
        ...
        //regular类型可能没有最后一个参数 需要判断一下
        Adventure man = adventures.get(advId);
        man.addPrice(price);
        //同时更新价值
        if (inputInfo.get(i).size() > 7) {
    ...
    public void OP5(int i)
    {
        ...
        man.subPrice(equipment.getPrice());
    ...
    public void OP9(int i)
    {
        ...
        for (Equipment item : equipments) {
            if (item.getName().equals(name) && item.getBecarried(item)) { //已经被携带的同名装备
                equipment1 = item;
                break;
            }
        }
